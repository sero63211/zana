...
| T927-desktop-ui-execution | `router_opencode_go_deepseek_v4_flash` with `max`, visible thread pending, lead `019fe396-2a2d-7493-ac98-a551ec7e4e1a`, silent | reserved `agent/t927-desktop-ui-execution`; generated local isolated worktree pending | exclusive `apps/desktop/src/**` plus `T927-desktop-ui-execution.md`; docs read-only; manifests/locks/Tauri/Core/Rust/Python/Android/state/ledger excluded | active, visible task dispatch pending | accepted desktop contracts + T910 package + `ui-execution-brief.md`; disjoint from Rust integration | focused Vitest/touched regression + TypeScript + ESLint + diff + contract/copy/a11y/security/performance/readability; zero dependency change; no build/app/browser/live/model/install |
