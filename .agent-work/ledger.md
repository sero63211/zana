...
| T900-api-system-runtime | `router_opencode_go_deepseek_v4_flash` with `max`, local visible task pending | `agent/t900-api-system-runtime` isolated worktree pending | single-owner system/runtime/model API boundary and exact tests/handoff | queued | canonical T007 domain/services | authenticated typed system profile/doctor, refresh and native pull routes; focused/full gates; no model start/download |
| T900-runtime-inference | `router_opencode_go_deepseek_v4_flash` with `max`, local visible task pending | `agent/t900-runtime-inference` isolated worktree pending | exact runtime inference adapter files/tests/handoff | queued | canonical runtime transport + instance inference protocol | bounded real Ollama/OpenAI-compatible inference code; fixture-only tests; no live model/server |
| T900-knowledge-providers | `router_opencode_go_deepseek_v4_flash` with `max`, local visible task pending | `agent/t900-knowledge-providers` isolated worktree pending | exact Docling/LanceDB adapter files/tests/handoff | queued | canonical knowledge models/limits | real optional provider adapters with honest unavailable state; no dependency install/model/network |
...
| T900-tauri-core-package | `router_opencode_go_deepseek_v4_flash` with `max`, local visible task pending | `agent/t900-tauri-core-package` isolated worktree pending | Tauri/core sidecar/package scripts and handoff | queued | canonical M0 Tauri supervisor | robust lightweight sidecar packaging/lifecycle/auth/path errors; no bundle/app launch |
