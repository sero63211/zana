...
| T900-resources-observability-api | `router_opencode_go_deepseek_v4_flash` with `max`, lead `019fe396-2a2d-7493-ac98-a551ec7e4e1a`, silent | reserved `agent/t900-resources-observability-api`; thread/worktree pending | resources/observability packages, isolated new API/schema/service files, focused tests and handoff; shared main/API/DB/jobs/desktop excluded | reserved; no writer yet | integrated conservative governor/snapshots/leases and bounded redacted local sinks | focused injected unit/router/static gates only; bounded safe projections, pagination/truncation/redaction/telemetry-off; no live/broad/background/export |
