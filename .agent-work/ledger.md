...
| T922A-rust-benchmark-engine | `router_opencode_go_deepseek_v4_flash` with `max`, visible local task pending, lead `019fe396-2a2d-7493-ac98-a551ec7e4e1a`, silent | reserved `agent/t922a-rust-benchmark-engine`; isolated worktree pending | new standalone `crates/zana-evaluation/**` plus exact handoff only; every existing/shared path excluded | queued for visible activation after reservation push | pinned T922 benchmark policy and T920 foundation only; later serial T922 integration | focused standalone Rust only; no dataset/model/network/provider/inference/training/app/device/root-workspace/broad |
