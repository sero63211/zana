...
| T900-desktop-product-flows | `router_opencode_go_deepseek_v4_flash` with `max`, lead `019fe396-2a2d-7493-ac98-a551ec7e4e1a`, silent | pending visible local Codex worktree | exclusive `apps/desktop/src/**`, focused source tests and handoff; no src-tauri/manifests/locks/Core | queued; no writer until worktree receipt | integrated desktop shell/models; documented frozen Core contracts | focused Vitest/TypeScript/ESLint and direct UX/resource review only; no app/browser/build/install/live |
| T900-runtime-native-tools | `router_opencode_go_deepseek_v4_flash` with `max`, lead `019fe396-2a2d-7493-ac98-a551ec7e4e1a`, silent | pending visible local Codex worktree | exact runtime inference/Ollama/OpenAI-compatible files/tests and handoff | queued; no writer until worktree receipt | bounded inference + trusted calculator/tool contracts | focused injected transport tests; Ruff/format/Pyright/import/diff; no live runtime/model/network |
| T900-instance-chat-memory-api | `router_opencode_go_deepseek_v4_flash` with `max`, lead `019fe396-2a2d-7493-ac98-a551ec7e4e1a`, silent | pending visible local Codex worktree | instances/memory plus new isolated API/service/schema files/tests/handoff; shared main/API/DB files excluded | queued; no writer until worktree receipt | persisted image/instance/memory schema and runtime/retrieval/tool foundations | focused injected unit/router tests; Ruff/format/Pyright/import/diff; no live model/runtime/API/app |
| T900-portability-product-api | `router_opencode_go_deepseek_v4_flash` with `max`, lead `019fe396-2a2d-7493-ac98-a551ec7e4e1a`, silent | pending visible local Codex worktree | portability plus new isolated API/service/schema files/tests/handoff; images/shared main/API/DB excluded | queued; no writer until worktree receipt | integrated OCI/archive/import/security and artifact registry | focused tiny archive/injected API tests; Ruff/format/Pyright/import/diff; no large/live/install/broad tests |
