...
| T921B-rust-compatibility-api | `router_opencode_go_deepseek_v4_flash` with `max`, visible local task pending, lead `019fe396-2a2d-7493-ac98-a551ec7e4e1a`, silent | reserved `agent/t921b-rust-compatibility-api`; isolated worktree pending from canonical `05c7eec` | only `crates/zana-core-server/**` plus exact new handoff; every shared core/other path excluded | queued after reservation push | accepted canonical DB/jobs/resources/doctor/runtime/settings modules; acquisition/observability later serial inputs | focused Rust server/router tests only; no external/live/model/runtime/provider/download/app/broad |
