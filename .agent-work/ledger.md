...
| T900-build-evaluation-image-execution | `router_opencode_go_deepseek_v4_flash` with `max`, first-class thread pending, lead `019fe396-2a2d-7493-ac98-a551ec7e4e1a`, silent | intended `agent/t900-build-evaluation-image-execution`; local Codex worktree pending creation; base `1900e9e` | dependency-complete persisted build analysis/baseline/approval/candidate evaluation/gates/image finalization plus exact build/image API, necessary DB contracts, focused tests/handoff | active; exclusive writer pending first-class task creation | integrated capability/runtime/knowledge/training/OCI/jobs/artifact/resource/API contracts | focused injected unit/API only; Ruff/format/Pyright/import/diff; lifecycle/transaction/digest/held-out/approval/cancel/restart/redaction/no-promotion review; no broad/live/model/training/app/build |
