...
| T921-rust-operational-core | `router_opencode_go_deepseek_v4_flash` with `max`, visible task pending creation, lead `019fe396-2a2d-7493-ac98-a551ec7e4e1a`, silent | reserved `agent/t921-rust-operational-core` from exact clean pushed base `b244ba5`; worktree pending | exclusive shared Rust operational plane and compatibility routes/tests; capability lifecycle, Python/TS/Tauri/Android/state/ledger excluded | active reservation; no product write before visible worktree receipt | accepted Python operational contracts plus T920 Rust foundation | injected/focused Rust only; no live runtime/model/provider/network/download/app/device/broad |
