...
| T922B-rust-artifact-domain | `router_opencode_go_deepseek_v4_flash` with `max`, visible task pending creation, lead `019fe396-2a2d-7493-ac98-a551ec7e4e1a`, silent | reserved `agent/t922b-rust-artifact-domain`; local isolated worktree pending | new standalone `crates/zana-artifacts/**` plus exact handoff only; every existing/shared path excluded | reserved; no writes/no push | accepted platform-neutral capability/build/Image/Instance/portability contracts; later serial T922 integration | focused standalone Rust only; no archive extraction/model/dataset/network/provider/inference/training/app/device/root-workspace/broad |
